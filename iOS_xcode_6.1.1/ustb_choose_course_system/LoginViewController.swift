    ...
    */
    ...
    @IBAction func localClassTableBtnClicked(sender: AnyObject) {
        ...
        if (cList.count == 0) || (tempStr == ""){
            var alert = UIAlertView(title: "提示", message: "你还没有添加本地课表，请登陆后添加", delegate: nil, cancelButtonTitle: "确定")
            alert.show()
            return
        }
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if segue.identifier == "loginSegue"{
            ...
            var vc = segue.destinationViewController as LocalClassTableViewController
            var list = sender as [String]
            vc.cList = []
            for str in list{
                if str != ""{
                    vc.cList.append(str)

...
class ClassTableViewController: UIViewController{
    ...
    @IBAction func moreBarBtnClicked(sender: AnyObject) {
        ...
        alert.addAction(UIAlertAction(title: "添加课表到本地", style: UIAlertActionStyle.Default, handler: { (UIAlertAction) -> Void in
            ...
            let unameStr = kalen.app.UserInfo.getInstance().username
            let semesterStr = self.semester.text
            ...
            if !isExist{
                ...
                userDefaults.setObject(join("\n", cList), forKey: "class_list")
            }
        }))
    ...
    override func viewDidLoad() {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), {
            ...
            self.jsonStr = kalen.app.HttpUtil.get(url, cookieStr: self.cookieData)
